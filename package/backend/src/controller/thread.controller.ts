...
export class ThreadController extends Controller {
    ...
    public async search(@Query() query: string, @Query() page?: number, @Query() limit?: number): Promise<ThreadPaginatedDTO> {
        const threads = await this.threadService.findByQuery(query, page, limit);
        const threadsDto = plainToInstance(ThreadPaginatedDTO, threads, { enableImplicitConversion: true, excludeExtraneousValues: true });
        return threadsDto;
    }
